...
GameInput::BuildPlayerCommand(const Engine::InputSystem &input) const {
  ...
  command.movement.x = static_cast<float>(IsKeyboardKeyDown(VK_RIGHT)) -
                       static_cast<float>(IsKeyboardKeyDown(VK_LEFT));
  command.movement.y = static_cast<float>(IsKeyboardKeyDown(VK_DOWN)) -
                       static_cast<float>(IsKeyboardKeyDown(VK_UP));
