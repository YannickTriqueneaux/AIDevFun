...
void DrawSquare(Engine::Renderer2D &renderer, Engine::Vector2 center,
                float halfSize, Engine::Color color) {
...
void DrawSquareOutline(Engine::Renderer2D &renderer, Engine::Vector2 center,
                       float halfSize, Engine::Color color) {
  const float left = center.x - halfSize;
  const float right = center.x + halfSize;
  const float top = center.y - halfSize;
...
void Player::Update(Engine::Gameplay::ObjectManager &objects, float deltaTime) {
  ...
  if (command.action == PlayerAction::Dash) {
    position += facing_ * DashDistance;
    ExecuteAction(command.action);
  } else if (command.action == PlayerAction::Reset) {
    Reset();
    position = {static_cast<float>(GameConfig::PlayAreaWidth) * 0.5f,
                static_cast<float>(GameConfig::PlayAreaHeight) * 0.5f};
    lastAction_ = "Reset";
  } else
    ExecuteAction(command.action);
  pulseTimeRemaining_ = std::max(0.0f, pulseTimeRemaining_ - deltaTime);
  position.x = std::clamp(position.x, PlayerHalfSize,
                          static_cast<float>(GameConfig::PlayAreaWidth) -
                              PlayerHalfSize);
  position.y = std::clamp(position.y, PlayerHalfSize,
                          static_cast<float>(GameConfig::PlayAreaHeight) -
                              PlayerHalfSize);
}

void Player::Render(Engine::Renderer2D &renderer,
                    const Engine::Gameplay::ObjectManager &objects) const {
  const auto *entity = GetOwner().Resolve(objects);
  if (entity == nullptr)
    return;
  ...
  if (pulseTimeRemaining_ > 0.0f) {
    ...
    DrawSquareOutline(renderer, position, halfSize, {255, 184, 77, alpha});
  }

  if (shieldEnabled_) {
    DrawSquareOutline(renderer, position, PlayerHalfSize + 10.0f,
                      {102, 191, 255, 255});
    DrawSquareOutline(renderer, position, PlayerHalfSize + 12.0f,
                      {0, 121, 241, 255});
  }

  DrawSquare(renderer, position, PlayerHalfSize, {86, 204, 157, 255});
  renderer.DrawLine(position, position + facing_ * 36.0f, 5.0f,
                    {245, 245, 245, 255});
}

void Player::RenderHud(Engine::Renderer2D &renderer) const {
  renderer.DrawText("MOVE: ARROW KEYS", {24.0f, 22.0f}, 20,
                    {200, 200, 200, 255});
  renderer.DrawText("Q: DASH   W: PULSE   E: SHIELD   R: RESET", {24.0f, 50.0f},
                    20, {200, 200, 200, 255});
  renderer.DrawText(std::string("LAST ACTION: ") + lastAction_, {24.0f, 84.0f},
                    20, {255, 203, 0, 255});
}
...
void Player::SaveState(Engine::Gameplay::StateWriter &writer) const {
  ...
  writer.Value(pulseTimeRemaining_);
  writer.Value(shieldEnabled_);
}

void Player::LoadState(Engine::Gameplay::StateReader &reader,
                       std::uint32_t version) {
  if (version != 1)
    throw std::runtime_error("Unsupported Player state version.");
  facing_ = reader.Value<Engine::Vector2>();
  movementSpeed_ = reader.Value<float>();
  pulseTimeRemaining_ = reader.Value<float>();
  shieldEnabled_ = reader.Value<bool>();
}
