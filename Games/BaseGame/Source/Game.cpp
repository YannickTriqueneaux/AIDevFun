...
namespace { constexpr auto PlayerEntityType=Engine::Gameplay::StableTypeID("BaseGame.PlayerEntity"); }

ProceduralGame::ProceduralGame()
{
    world_.RegisterComponent({Player::Type,"Player",[](Engine::Gameplay::ObjectRef<Engine::Gameplay::Entity> owner){return NEW_OBJECT(Player,owner);}});
    ...
    entity->transform.position={static_cast<float>(GameConfig::PlayAreaWidth)*0.5f,static_cast<float>(GameConfig::PlayAreaHeight)*0.5f};
...
void ProceduralGame::Update(
    ...
    float deltaTime)
{
    if(auto* player=player_.Resolve(world_.Objects()))player->SetCommand(inputBindings_.BuildPlayerCommand(input));
...
void ProceduralGame::Render(Engine::RenderContext& context) const
{
    ...
    if(const auto* player=player_.Resolve(world_.Objects())){player->Render(renderer,world_.Objects());player->RenderHud(renderer);}
#if !defined(GAME_RELEASE_BUILD)
    renderer.DrawFramesPerSecond(renderer.GetWidth() - 100, 20);
#endif
}

std::vector<std::byte> ProceduralGame::SaveResumeState() const{return world_.Save();}
void ProceduralGame::ResumeFromState(std::span<const std::byte> state)
{
    ...
    player_ = Engine::Gameplay::ObjectRef<Player>(playerEntity->components.front());
}
...
void ProceduralGame::SerializeAutoTestState(Engine::Serializer& serializer)
{
    ...
    float x=entity?entity->transform.position.x:0.0f;float y=entity?entity->transform.position.y:0.0f;int entityIndex=static_cast<int>(playerEntity_.GetID().index);int entityVersion=static_cast<int>(playerEntity_.GetID().version);int objectCount=static_cast<int>(world_.Objects().LiveCount());
    serializer.Value("player.position.x",x);serializer.Value("player.position.y",y);serializer.Value("player.entity.index",entityIndex);serializer.Value("player.entity.version",entityVersion);serializer.Value("world.objectCount",objectCount);
    (void)player;
}
#endif
