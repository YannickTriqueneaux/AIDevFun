...
namespace
{
    ...
    {
    public:
        void Value(std::string_view name, bool& value) override { values[std::string(name)] = value; }
        void Value(std::string_view name, int& value) override { values[std::string(name)] = value; }
        void Value(std::string_view name, float& value) override { values[std::string(name)] = value; }
        void Value(std::string_view name, std::string& value) override { values[std::string(name)] = value; }
...
int main(int argc, char** argv)
{
    try
    {
        ...
        throw std::runtime_error("HotReloadTests must not run without ENGINE_AUTOTESTS.");
#else
        if (argc != 3)
        {
            throw std::runtime_error("Usage: HotReloadTests <Game.dll> <shadow-directory>");
        }
        ...
        Require(game.GetReloadStatus().find("Reloaded Game generation 2") != std::string::npos,
