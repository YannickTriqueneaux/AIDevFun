  ...
  try {
    ...
    const std::filesystem::path executableDirectory =
        std::filesystem::absolute(argc > 0 ? std::filesystem::path(argv[0])
                                           : std::filesystem::path{})
            .parent_path();
    Engine::Logger::Initialize(executableDirectory / "Logs" / "GameHost.log");
    ...
    if (!std::filesystem::exists(activeGameRoot / "Source" /
                                 "GameModule.cpp")) {
      throw std::runtime_error(
          "Configured active Game project does not exist: " +
          activeGameRoot.string());
    }
    Engine::Logger::Info("Active Game project: " + activeGameRoot.string());

    if (recoveryMode) {
      Engine::Logger::Warning(
          "GameHost entered crash recovery mode; Game.dll will not load.");
      GameToolService tools(nullptr, activeGameRoot,
                            executableDirectory.parent_path(),
                            executableDirectory, true);
      Engine::Logger::Info("Recovery tools are ready for the AI Assistant.");
      {
        std::ofstream request(executableDirectory / "AIRecovery.prompt",
                              std::ios::binary | std::ios::trunc);
        request << "The Game entered crash recovery mode after a crash. "
                   "Inspect the crash "
      ...
      Engine::Logger::Info("AI Assistant requested a repaired Game launch.");
      return 42;
    }

    ReloadableGame game(executableDirectory / "Game.dll",
    ...
    Engine::ApplicationConfig config{
        ...
        .focusWindowTitle =
            std::string(MAKE_YOUR_OWN_GAME_AI_GAME_PROJECT) + " - AI Assistant",
        .verticalSync = gameConfig.verticalSync};

    Engine::Application application(std::move(config));
    {
      GameToolService tools(&game, activeGameRoot,
                            executableDirectory.parent_path(),
                            executableDirectory);
