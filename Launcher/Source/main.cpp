...
public:
  explicit Process(const std::filesystem::path &executable,
    ...
    if (!CreateProcessW(executable.c_str(), commandLine.data(), nullptr,
                        ...
                        executable.parent_path().c_str(), &startupInfo,
                        &processInfo_)) {
      throw std::runtime_error("Failed to start " +
                               executable.filename().string());
    }

    Engine::Logger::Info("Started process " + executable.filename().string() +
  ...
  void RequestWindowClose() const {
    EnumWindows(
        [](HWND window, LPARAM parameter) -> BOOL {
          const DWORD targetProcessId = static_cast<DWORD>(parameter);
  ...
  try {
    const std::filesystem::path executableDirectory =
        std::filesystem::absolute(argc > 0 ? std::filesystem::path(argv[0])
                                           : std::filesystem::path{})
            .parent_path();
    Engine::Logger::Initialize(executableDirectory / "Logs" / "Launcher.log");
    Engine::CrashDiagnostics::Install(executableDirectory / "Crashes",
                                      "Launcher");
    {
      std::error_code error;
      std::filesystem::remove(executableDirectory / "AIRecovery.prompt", error);
    }
#if !defined(NDEBUG)
    if (argc > 1 && std::string_view(argv[1]) == "--test-crash-diagnostics") {
#if defined(_WIN32)
    ...
    while (keepRunning) {
      ...
      Engine::Logger::Info("GameHost exited with code " +
      ...
      const int choice = MessageBoxW(
          ...
          MB_ICONERROR | MB_YESNO | MB_DEFBUTTON1 | MB_SETFOREGROUND);
      if (choice != IDYES) {
      ...
      Engine::Logger::Warning("Starting GameHost in AI crash recovery mode.");
      Process recoveryHost(executableDirectory / "GameHost.exe", L"--recovery");
