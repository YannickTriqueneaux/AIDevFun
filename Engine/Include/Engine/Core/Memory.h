...
namespace Engine::Memory
{
    ...
    struct AllocationInfo { std::uint32_t index = 0; std::uint32_t version = 0; };

    ENGINE_API void* Allocate(std::size_t size, std::size_t alignment);
    ENGINE_API void Release(void* memory, std::size_t size, std::size_t alignment) noexcept;
    ENGINE_API void Release(void* memory) noexcept;
    ENGINE_API BucketStats GetBucketStats(std::size_t size, std::size_t alignment);
    ENGINE_API AllocationInfo GetAllocationInfo(const void* memory, std::size_t size, std::size_t alignment);
...
#define NEW_MEMORY(Type, ...) \
    ::Engine::Memory::New<Type>(__VA_ARGS__)

#define DELETE_MEMORY(MemoryOwner) \
    ::Engine::Memory::Delete(MemoryOwner)
