...
namespace Engine::Gameplay
{
    ...
    ENGINE_API void* AllocateObjectMemory(TypeID storageType, std::size_t size, std::size_t alignment);
    ENGINE_API void ReleaseObjectMemory(TypeID storageType, void* memory, std::size_t size, std::size_t alignment) noexcept;
    ...
    struct ObjectAllocationInfo { std::uint32_t index = 0; std::uint32_t version = 0; };
    ENGINE_API ObjectAllocationInfo GetObjectAllocationInfo(TypeID storageType, const Object* object);

    template<class T>
    consteval TypeID ObjectStorageTypeID()
    {
#if defined(_MSC_VER)
    ...
    ObjectPtr NewObject(Arguments&&... arguments)
    {
        ...
        try
        {
            ...
            return ObjectPtr(object, ObjectDeleter{
                storageType,
                sizeof(T),
                alignof(T),
                [](Object* value) noexcept { static_cast<T*>(value)->~T(); }});
        }
        catch (...)
        {
...
#define NEW_OBJECT(Type, ...) \
    ::Engine::Gameplay::NewObject<Type>(__VA_ARGS__)

#define DELETE_OBJECT(ObjectOwner) \
    (ObjectOwner).reset()
