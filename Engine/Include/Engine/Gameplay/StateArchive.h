...
public:
  ...
  void String(const std::string &value) {
    Value(static_cast<std::uint32_t>(value.size()));
...
public:
  ...
  T Value() {
    if (remaining() < sizeof(T))
      throw std::runtime_error("Truncated gameplay state.");
    T v{};
    std::memcpy(&v, data_.data() + offset_, sizeof(T));
    offset_ += sizeof(T);
    return v;
  }
  std::string String() {
    const auto size = Value<std::uint32_t>();
    const auto b = Bytes(size);
    return {reinterpret_cast<const char *>(b.data()), b.size()};
  }
  std::span<const std::byte> Bytes(std::size_t size) {
    if (size > remaining())
      throw std::runtime_error("Truncated gameplay state payload.");
    auto r = data_.subspan(offset_, size);
    offset_ += size;
    return r;
  }
  ...
  void RequireEnd() const {
    if (remaining() != 0)
      throw std::runtime_error("Gameplay state has trailing data.");
  }
