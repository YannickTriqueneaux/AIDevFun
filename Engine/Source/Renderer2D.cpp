...
void Renderer2D::DrawLine(Vector2 start, Vector2 end, float thickness,
                          Color color) {
  DrawLineEx(ToRaylibVector(start), ToRaylibVector(end), thickness,
             ToRaylibColor(color));
}

void Renderer2D::DrawText(std::string_view text, Vector2 position, int fontSize,
                          Color color) {
  const std::string terminatedText(text);
  ::DrawText(terminatedText.c_str(), static_cast<int>(position.x),
             static_cast<int>(position.y), fontSize, ToRaylibColor(color));
}
...
int Renderer2D::GetHeight() const { return GetScreenHeight(); }
} // namespace Engine
