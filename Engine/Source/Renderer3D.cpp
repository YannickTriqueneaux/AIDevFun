...
namespace
{
    ...
    {
        return {
            ToRaylibVector(camera.position),
            ToRaylibVector(camera.target),
            ToRaylibVector(camera.up),
            camera.fieldOfView,
...
namespace Engine
{
    ...
    void Renderer3D::DrawCube(Vector3 position, Vector3 size, Color color)
    {
        assert(active_);
        DrawCubeV(
            ToRaylibVector(position),
            ToRaylibVector(size),
            ToRaylibColor(color));
    }

    void Renderer3D::DrawCubeOutline(
        Vector3 position,
        Vector3 size,
        Color color)
    {
