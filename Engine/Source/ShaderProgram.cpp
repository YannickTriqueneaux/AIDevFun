...
namespace
{
    int GetLocation(::Shader shader, std::string_view name)
    {
        ...
        return GetShaderLocation(
            shader,
            terminatedName.c_str());
    }
...
namespace Engine
{
    ...
    bool ShaderProgram::LoadFromMemory(
        ...
        std::string_view fragmentSource)
    {
        ...
        implementation_->shader = LoadShaderFromMemory(
            SourceOrNull(vertex),
            SourceOrNull(fragment));
        return IsValid();
    }
    ...
    bool ShaderProgram::IsValid() const
    {
        return implementation_ &&
               ::IsShaderValid(implementation_->shader);
    }
    ...
    void ShaderProgram::SetFloat(std::string_view name, float value) const
    {
        if (IsValid())
        {
            SetShaderValue(
                implementation_->shader,
                GetLocation(implementation_->shader, name),
                &value,
    ...
    void ShaderProgram::SetColor(std::string_view name, Color value) const
    {
        if (IsValid())
        {
            ...
            const float values[]{
                value.red * ByteToUnit,
                value.green * ByteToUnit,
                value.blue * ByteToUnit,
                value.alpha * ByteToUnit
            };
            SetShaderValue(
                implementation_->shader,
                GetLocation(implementation_->shader, name),
