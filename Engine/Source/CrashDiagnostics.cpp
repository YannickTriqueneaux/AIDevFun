...
// DbgHelp requires the Windows declarations. Keep these include blocks separate
// so clang-format does not alphabetize DbgHelp ahead of Windows.
#include <Windows.h>
...
std::wstring BuildBasePath() {
  ...
  wchar_t name[256]{};
  swprintf_s(name, L"%s_%04u%02u%02u_%02u%02u%02u_%lu_%u", ProcessName.c_str(),
             time.wYear, time.wMonth, time.wDay, time.wHour, time.wMinute,
             time.wSecond, GetCurrentProcessId(), Sequence.fetch_add(1));
...
USHORT CaptureCrashFrames(EXCEPTION_POINTERS *exceptionPointers, void **frames,
                          USHORT capacity) {
  ...
  while (count < capacity &&
         StackWalk64(machineType, process, thread, &frame, &context, nullptr,
...
void WriteSymbolizedFrame(HANDLE file, USHORT index, void *frameAddress) {
  const HANDLE process = GetCurrentProcess();
  const DWORD64 address = reinterpret_cast<DWORD64>(frameAddress);
  ...
  const bool hasSymbol =
      SymbolsInitialized &&
      SymFromAddr(process, address, &symbolDisplacement, symbol) != FALSE;

  IMAGEHLP_MODULE64 module{};
  module.SizeOfStruct = sizeof(module);
  const bool hasModule = SymbolsInitialized &&
  ...
  char output[2048]{};
  int length =
      sprintf_s(output, "  #%02u %p %s%s%s", index, frameAddress,
                hasModule && module.ModuleName[0] != '\0' ? module.ModuleName
                                                          : "<unknown-module>",
                hasSymbol ? "!" : "", hasSymbol ? symbol->Name : "");
  if (hasSymbol && symbolDisplacement != 0) {
    length += sprintf_s(output + length, sizeof(output) - length, "+0x%llX",
                        symbolDisplacement);
  }
  if (hasLine && line.FileName) {
    length += sprintf_s(output + length, sizeof(output) - length, " (%s:%lu",
                        line.FileName, line.LineNumber);
    if (lineDisplacement != 0) {
      length += sprintf_s(output + length, sizeof(output) - length, "+0x%lX",
                          lineDisplacement);
    }
    length += sprintf_s(output + length, sizeof(output) - length, ")");
  }
  sprintf_s(output + length, sizeof(output) - length, "\r\n");
  WriteText(file, output);
}

void WriteTextReport(const std::wstring &path, const char *reason,
  ...
  void *frames[48]{};
  const USHORT frameCount = CaptureCrashFrames(exceptionPointers, frames, 48);
  sprintf_s(report,
            "process=%ls\r\npid=%lu\r\nthread=%lu\r\n"
            "reason=%s\r\nexception_code=0x%08lX\r\n"
            "exception_address=%p\r\n"
            "callstack=\r\n",
            ProcessName.c_str(), GetCurrentProcessId(), GetCurrentThreadId(),
            reason, exceptionCode, exceptionAddress);
...
void WriteMiniDump(const std::wstring &path,
                   EXCEPTION_POINTERS *exceptionPointers) {
  const HANDLE file =
      CreateFileW(path.c_str(), GENERIC_WRITE, FILE_SHARE_READ, nullptr,
                  CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
  ...
  MINIDUMP_EXCEPTION_INFORMATION exceptionInformation{GetCurrentThreadId(),
                                                      exceptionPointers, FALSE};
  MiniDumpWriteDump(
...
void RecordCrash(const char *reason,
...
int OnCrtReport(int reportType, wchar_t *, int *) {
  if (reportType == _CRT_ASSERT || reportType == _CRT_ERROR) {
    RecordCrash(reportType == _CRT_ASSERT ? "crt_assertion" : "crt_error");
  }
  return FALSE;
}
} // namespace
...
namespace Engine {
void CrashDiagnostics::Install(const std::filesystem::path &outputDirectory,
  ...
  const std::filesystem::path runtimeDirectory = outputDirectory.parent_path();
  ...
  SymSetOptions(SymGetOptions() | SYMOPT_DEFERRED_LOADS | SYMOPT_LOAD_LINES |
                SYMOPT_UNDNAME);
  SymbolsInitialized = SymInitializeW(GetCurrentProcess(),
                                      symbolSearchPath.c_str(), TRUE) != FALSE;
...
namespace Engine {
void CrashDiagnostics::Install(const std::filesystem::path &,
                               std::string_view) {}
} // namespace Engine
