...
ImGuiCond ToImGuiCondition(Engine::UiCondition condition) {
  return condition == Engine::UiCondition::Always ? ImGuiCond_Always
                                                  : ImGuiCond_FirstUseEver;
}

ImVec4 ToImGuiColor(Engine::Color color) {
  constexpr float InverseByte = 1.0f / 255.0f;
  return {color.red * InverseByte, color.green * InverseByte,
          color.blue * InverseByte, color.alpha * InverseByte};
}
} // namespace
...
UiSystem::~UiSystem() { rlImGuiShutdown(); }

void UiSystem::BeginFrame() { rlImGuiBegin(); }

void UiSystem::EndFrame() { rlImGuiEnd(); }
...
void UiSystem::SetNextWindowSize(Vector2 size, UiCondition condition) {
  ImGui::SetNextWindowSize({size.x, size.y}, ToImGuiCondition(condition));
}

bool UiSystem::BeginPanel(std::string_view title, bool movable, bool resizable,
                          bool decorated) {
...
void UiSystem::EndPanel() { ImGui::End(); }
...
void UiSystem::TextWrapped(std::string_view text, float wrapWidth,
                           Color color) {
  ImGui::PushStyleColor(ImGuiCol_Text, ToImGuiColor(color));
...
void UiSystem::Separator() { ImGui::Separator(); }

void UiSystem::Spacing() { ImGui::Spacing(); }

void UiSystem::SetKeyboardFocusHere() { ImGui::SetKeyboardFocusHere(); }

void UiSystem::ScrollToBottom() { ImGui::SetScrollHereY(1.0f); }
...
float UiSystem::GetCursorX() const { return ImGui::GetCursorPosX(); }

void UiSystem::SetCursorX(float localX) { ImGui::SetCursorPosX(localX); }
