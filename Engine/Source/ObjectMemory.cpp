...
namespace Engine::Gameplay
{
    ...
    void* AllocateObjectMemory(TypeID storageType, std::size_t size, std::size_t alignment)
    {
        std::scoped_lock lock(poolMutex);
        const PoolKey key{storageType, size, alignment};
        auto& pool = pools[key];
        if (!pool) pool = std::make_unique<Pool>(key);
        bool recycled = false;
        void* result = pool->memory.Allocate(recycled);
        ++pool->stats.liveObjects;
        ++pool->stats.totalAllocations;
        if (recycled) ++pool->stats.recycledAllocations;
        pool->stats.availableBlocks = pool->memory.AvailableCount();
        return result;
    }

    void ReleaseObjectMemory(TypeID storageType, void* memory, std::size_t size, std::size_t alignment) noexcept
    {
        if (!memory) return;
        std::scoped_lock lock(poolMutex);
        const auto found = pools.find({storageType, size, alignment});
        if (found == pools.end()) std::terminate();
    ...
    ObjectPoolStats GetObjectPoolStats(TypeID storageType)
    {
        ...
        {
            if (key.storageType != storageType) continue;
    ...
    ObjectAllocationInfo GetObjectAllocationInfo(TypeID storageType, const Object* object)
    {
        std::scoped_lock lock(poolMutex);
        Detail::SparseMemoryPool* owningPool = Detail::SparseMemoryPool::PoolOf(object);
        for (const auto& [key, pool] : pools)
        {
        ...
        throw std::invalid_argument("Object does not belong to the requested type pool.");
    }
