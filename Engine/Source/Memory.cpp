...
struct BucketHash {
  std::size_t operator()(const BucketKey &key) const noexcept {
    return key.size ^
           (key.alignment + 0x9e3779b9u + (key.size << 6u) + (key.size >> 2u));
  }
};
struct Bucket {
  BucketKey key;
  Detail::SparseMemoryPool pool;
  BucketStats stats;
  Bucket(BucketKey value) : key(value), pool(value.size, value.alignment) {}
};
...
AllocationInfo GetAllocationInfo(const void *memory, std::size_t size,
                                 std::size_t alignment) {
  std::scoped_lock lock(bucketMutex);
