...
namespace Engine
{
    OpenAICostEstimate EstimateOpenAICost(
        ...
        const OpenAITokenUsage& usage)
    {
        ...
        if (!pricing.IsConfigured() ||
            (model != pricing.model &&
             !model.starts_with(snapshotPrefix)))
        {
            return {};
        }
        ...
        const std::uint64_t uncachedTokens =
            usage.inputTokens - cachedTokens;
        const bool longContext =
            pricing.longContextThreshold > 0 &&
            usage.inputTokens > pricing.longContextThreshold;
        const double inputMultiplier = longContext
            ? pricing.longContextInputMultiplier
            : 1.0;
        const double outputMultiplier = longContext
            ? pricing.longContextOutputMultiplier
            : 1.0;

        const double inputCost =
            (static_cast<double>(uncachedTokens) *
                 pricing.inputUsdPerMillion +
             static_cast<double>(cachedTokens) *
                 pricing.cachedInputUsdPerMillion) /
            1'000'000.0 * inputMultiplier;
        const double outputCost =
            static_cast<double>(usage.outputTokens) *
            pricing.outputUsdPerMillion /
            1'000'000.0 * outputMultiplier;
