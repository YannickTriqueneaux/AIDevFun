...
void ObjectManager::BindReserved(ObjectID id, ObjectPtr object) {
  if (!object || id.index >= impl_->slots.size())
    throw std::invalid_argument("Invalid object binding.");
  auto &slot = impl_->slots[id.index];
  if (!slot.reserved || slot.version != id.version || slot.object)
    throw std::logic_error("ObjectID is not reserved.");
  object->id_ = id;
...
void ObjectManager::Restore(ObjectID id, ObjectPtr object) {
  ...
  if (impl_->slots.size() <= id.index)
    impl_->slots.resize(static_cast<std::size_t>(id.index) + 1);
  auto &slot = impl_->slots[id.index];
  if (slot.object || slot.reserved)
    throw std::logic_error("Duplicate restored ObjectID.");
...
void ObjectManager::RejectRestored(ObjectID id) {
  ...
  auto &slot = impl_->slots[id.index];
  if (slot.object || slot.reserved)
    throw std::logic_error("Cannot reject an occupied restored ObjectID.");
...
void ObjectManager::Destroy(ObjectID id) {
  if (id.index >= impl_->slots.size())
    return;
  auto &slot = impl_->slots[id.index];
  if (slot.version != id.version || (!slot.object && !slot.reserved))
    return;
